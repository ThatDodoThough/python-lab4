...
def saveListToFile():
    ...
    try:
        ...
        #open sql connection
        connection = pymysql.connect(user="root",password="dodo",database="task",host="localhost")
        cursor = connection.cursor()
        sql = """
            INSERT INTO task (todo)
            VALUES (%s);
        """
        ...
        for single_task in tasks_list:
            ...
            cursor.execute(sql, (single_task,))
            connection.commit()

        #chiudi sql connection and cursor
        cursor.close()
        connection.close()
...
if __name__ == '__main__':
    ...
    try:
        ...
        for tupla in tabella:
            tasks_list.append(tupla[1])
        cursor.close()
        connection.close()
        #fine sql
    ...
    updater = Updater(token='567551135:AAFJWKVZ8JS0raa_cVQa4c0GoOkTep1cK8k')
